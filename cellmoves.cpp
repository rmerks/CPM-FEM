...
// cellular potts model: one Monte Carlo step
{
	...
	for(i=0;i<NRsteps;i++)
	{
		...
		if((Rloc<R) && (Rloc >20))
		{
			...
			if(go_on)
			{
        		dH = calcdH(pv,pn,csize,xt,xs,pick,ttag,stag,incr);
			
			//Energy that is needed to do a retraction
			double dHdiss = 0.0;
			if(par.LAMBDADISS>0){
		//retraction
		if(ttag){dHdiss += par.COLLAGEN/par.LAMBDADISS;}
		//extension
		//if(stag){dHdiss += -par.LAMBDADISS/(1+par.COLLAGEN/par.LAMBDADISS);}
			}

        		prob = exp(-(dH+dHdiss)/(par.MOTILITY));
			//cout << endl << "dHdiss " << dHdiss << endl;
			//cout << "dH " << dH << endl;
			//cout << "prob " << prob << endl;
